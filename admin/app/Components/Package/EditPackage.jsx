        ...
        {list.map((item, i) => (
          ...
          >
            ...
            <button
              type="button"
              onClick={() => onRemove(i)}
              className="text-gray-400 hover:text-rose-500 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-rose-400"
...
export default function EditPackage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setLoading(true);
        const { data } = await axios.get(`${apiBase}/package/${id}`, { withCredentials: true });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Do NOT set Content-Type header for multipart/form-data; let the browser/axios set the boundary
      await axios.put(`${apiBase}/package/edit-package/${id}`, formData, {
