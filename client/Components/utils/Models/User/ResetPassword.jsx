...
export default function ResetPassword({ setRoute, setOpen }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(endpoint, { email, otp: code, password }, { withCredentials: true, timeout: 15000 });
  ...
  return (
    <div className="w-full max-w-md mx-auto px-4 py-6 sm:px-6 sm:py-8 bg-white dark:bg-slate-900 rounded-2xl shadow-lg transition-all duration-300">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <button type="submit" disabled={!isFormValid()} aria-busy={loading} aria-disabled={!isFormValid()} className={`mt-2 w-full ${styles.button} ${!isFormValid() ? "opacity-50 cursor-not-allowed" : ""}`}>
