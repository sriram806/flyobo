...
const Header = ({ open, setOpen, activeItem, route, setRoute }) => {
    ...
    // Close notif on outside click / ESC
    useEffect(() => {
        const onDocClick = (e) => {
    ...
    // Close sidebar when clicking outside or pressing ESC (mobile)
    useEffect(() => {
        const onDocClick = (e) => {
    ...
    // keep unread count fresh on mount and when user changes
    useEffect(() => {
    ...
    const fetchUnreadCount = async () => {
        ...
        try {
            ...
            if (!token) return; // avoid unauthenticated calls
    ...
    useEffect(() => {
        if (openNotif) {
            fetchNotifications();
            // Focus first notif for keyboard users after dropdown opens
    ...
    // helper for formatting small date
    ...
    return (
        ...
        transition-colors duration-300"
            >
                <div className="max-w-7xl mx-auto px-4 lg:px-8">
                    ...
                    <div className="hidden md:flex items-center justify-between h-20">
                        ...
                        ring-1 ring-sky-50 dark:ring-sky-900/40"
                                        >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {unreadCount > 0 && (
                                                        <button
                                                            ...
                                                            className="text-xs text-sky-600 hover:text-sky-800 dark:text-sky-400 dark:hover:text-sky-300 focus:outline-none"
                                                        >
                                                            Mark all
                                                        </button>
                                                    )}
                                                    <button
                                                        ...
                                                        className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-2"
                                                    >
                                  ...
                                  ${n?.status === "unread" ? "bg-sky-50 dark:bg-sky-900/20" : "hover:bg-gray-50 dark:hover:bg-gray-800"}`}
                                                            ...
                                                            >
                                                                <span
                                                                    className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${n?.status === "read" ? "bg-gray-400" : "bg-sky-500"
                                                                        }`}
                                                                    aria-hidden
                                                                />
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        {isNew && (
                                                                            <span className="text-[10px] bg-sky-500 text-white px-2 py-0.5 rounded-full">New</span>
                                                                        )}
                                                                    </div>
            ...
            {/* Auth modals (unchanged behavior) */}
            {open && (
                <CustomModel
                    open={open}
                    setOpen={setOpen}
