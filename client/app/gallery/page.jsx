                ...
                <div key={(it._id || it.image || idx) + idx} className="mb-4 break-inside-avoid-column">
                  <div className="group relative overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-800">
                    <button onClick={() => openAt(idx)} className="block w-full text-left">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={it.image} alt={it.title || `Photo ${idx + 1}`} className="w-full h-auto object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                    </button>
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-3 text-white">
                      <div className="text-xs text-white/80">{new Date(it.createdAt || it.date).toLocaleDateString()}</div>
                    </div>
                  </div>
                </div>
              ))}
