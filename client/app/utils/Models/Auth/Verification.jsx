...
const Verification = ({ setOpen, setRoute }) => {
  ...
  const handleChange = (index, raw) => {
    setError("");
    const value = raw.replace(/\D/g, "").slice(0, 1); // numeric single char
    setValues((prev) => {
      const next = [...prev];
      next[index] = value;
      return next;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = `${base}/auth/verify-otp`;
      ...
      // Optionally inspect data.message for success feedback
      if (data?.message) {
        toast.success(data.message);
      }
      // Close modal or route to next step
  ...
  const handleResend = async () => {
    try {
      ...
      const base = API_URL.replace(/\/$/, "");
      // Adjust this endpoint to match your backend for resending the OTP
      const endpoint = `${base}/auth/resend-otp`;
  ...
  return (
    <div className="w-full max-w-md mx-auto px-4 py-6 sm:px-6 sm:py-8 bg-white dark:bg-slate-900 rounded-2xl shadow-lg transition-all duration-300">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-6">
        ...
        {error && (
          <p className={`${styles.errorText} mt-4`} aria-live="assertive">{error}</p>
        )}

        <button
          type="submit"
          disabled={!isComplete || loading}
          className={`mt-6 w-full ${styles.button}`}
        >
